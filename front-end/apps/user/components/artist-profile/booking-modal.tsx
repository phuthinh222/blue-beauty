...
export function BookingModal({ artistName, services, initialService, onClose }: BookingModalProps) {
        ...
        <div key={n} className="flex items-center gap-2">
          <div
            className={`flex size-6 items-center justify-center rounded-full text-xs font-bold transition ${
              n <= current ? "bg-[#257CBA] text-white" : "bg-slate-200 text-slate-400"
            }`}
          >
            {n}
          </div>
          {n < 2 && (
            <div className={`h-px w-8 transition ${n < current ? "bg-[#257CBA]" : "bg-slate-200"}`} />
          )}
        </div>
      ))}
    </div>
...
function InfoStep({ artistName, form, today, isValid, onChange, onNext, onClose }: InfoStepProps) {
  return (
    <>
      ...
      <div className="space-y-4 px-6 py-5">
        ...
        <div>
          <label className="mb-1.5 flex items-center gap-1.5 text-xs font-semibold text-slate-600">
            <CalendarDays className="size-3.5 text-[#257CBA]" />
            Ngày đặt lịch <span className="text-red-400">*</span>
          </label>
          <input
            type="date" min={today} value={form.date}
            onChange={(e) => onChange("date", e.target.value)}
            className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm text-slate-800 outline-none transition focus:border-[#257CBA] focus:ring-2 focus:ring-[#257CBA]/20"
          />
        </div>
        ...
        <div>
          <label className="mb-1.5 flex items-center gap-1.5 text-xs font-semibold text-slate-600">
            <Clock className="size-3.5 text-[#257CBA]" />
            ...
            {TIME_SLOTS.map((slot) => (
              <button key={slot} type="button" onClick={() => onChange("time", slot)}
                className={`rounded-lg border px-3 py-1.5 text-xs font-medium transition ${
                  form.time === slot
                    ? "border-[#257CBA] bg-[#257CBA] text-white"
                    : "border-slate-200 text-slate-600 hover:border-[#257CBA] hover:text-[#257CBA]"
                }`}
              >
                {slot}
              </button>
        ...
        <div>
          <label className="mb-1.5 flex items-center gap-1.5 text-xs font-semibold text-slate-600">
            <MapPin className="size-3.5 text-[#257CBA]" />
            Địa điểm <span className="text-red-400">*</span>
          </label>
          <input
            type="text" placeholder="Nhập địa chỉ cụ thể..." value={form.location}
            onChange={(e) => onChange("location", e.target.value)}
            className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm text-slate-800 placeholder-slate-400 outline-none transition focus:border-[#257CBA] focus:ring-2 focus:ring-[#257CBA]/20"
          />
        </div>
        ...
        <div>
          <label className="mb-1.5 flex items-center gap-1.5 text-xs font-semibold text-slate-600">
            <FileText className="size-3.5 text-[#257CBA]" />
            Ghi chú cho thợ
          </label>
          <textarea
            ...
            rows={3}
            className="w-full resize-none rounded-xl border border-slate-200 px-4 py-2.5 text-sm text-slate-800 placeholder-slate-400 outline-none transition focus:border-[#257CBA] focus:ring-2 focus:ring-[#257CBA]/20"
          />
        </div>

        <button
          onClick={onNext} disabled={!isValid}
          className="w-full rounded-xl bg-[#257CBA] py-3 text-sm font-semibold text-white transition hover:bg-[#1e6aa0] disabled:cursor-not-allowed disabled:opacity-40"
        >
          Tiếp theo →
        </button>
      </div>
    ...
    <>
      ...
      <div className="px-6 py-5">
            ...
            return (
              <button
                ...
                className={`w-full rounded-xl border p-4 text-left transition ${
                  isSelected
                    ? "border-[#257CBA] bg-[#257CBA]/5 ring-1 ring-[#257CBA]"
                    : "border-slate-200 hover:border-[#257CBA]/50 hover:bg-slate-50"
                }`}
              >
                {/* Header: tên + giá */}
                <div className="flex items-start justify-between gap-2">
                  ...
                  <p className="shrink-0 text-base font-bold text-[#257CBA]">
                    {svc.price.toLocaleString("vi-VN")}₫
                  </p>
                </div>

                {/* Bao gồm (luôn hiển thị) */}
                ...
                {/* Mô tả chi tiết — chỉ hiện khi chọn */}
                {isSelected && (
                  <div className="mt-3 rounded-lg border border-[#257CBA]/20 bg-white p-3">
                    ...
                    <ul className="space-y-1.5">
                      {svc.description.map((line) => (
                        <li key={line} className="flex items-start gap-2 text-xs text-slate-600">
                          <svg className="mt-0.5 size-3.5 shrink-0 text-[#257CBA]" viewBox="0 0 20 20" fill="currentColor">
        ...
        <button
          ...
          className="mt-4 w-full rounded-xl bg-[#257CBA] py-3 text-sm font-semibold text-white transition hover:bg-[#1e6aa0] disabled:cursor-not-allowed disabled:opacity-40"
        >
          Xác nhận đặt lịch
        </button>
      </div>
      ...
      {service && (
        <div className="mt-5 w-full rounded-xl border border-slate-200 bg-slate-50 p-4 text-left">
          ...
          <div className="mt-2 space-y-1.5 text-sm text-slate-700">
            ...
            <p><span className="text-slate-400">Giá:</span> <span className="font-semibold text-[#257CBA]">{service.price.toLocaleString("vi-VN")}₫</span></p>
      ...
      <div className="mt-6 flex w-full gap-3">
        ...
        {service && (
          <button
            ...
            className="flex-1 rounded-xl bg-[#257CBA] py-3 text-sm font-semibold text-white transition hover:bg-[#1e6aa0]"
          >
