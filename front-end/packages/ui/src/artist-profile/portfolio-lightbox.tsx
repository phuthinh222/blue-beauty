      ...
      >
        <div className="flex items-center justify-between border-b border-slate-100 px-8 py-5">
          <div className="flex items-center gap-2">
            ...
            <span className="text-base text-slate-400">({item.images.length} ảnh)</span>
          </div>
          <button
            onClick={onClose}
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between border-t border-slate-100 px-8 py-4">
            <button
              onClick={prev}
              disabled={page === 0}
              className="flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium text-slate-600 transition hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-30"
            >
              ...
              {Array.from({ length: totalPages }, (_, i) => (
                <button
                  ...
                  className={`size-2.5 rounded-full transition ${
                    i === page ? "scale-125 bg-brand" : "bg-slate-300 hover:bg-slate-400"
                  }`}
                />
              ))}
            </div>
